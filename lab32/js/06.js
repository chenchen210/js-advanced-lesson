...
ES5的继承，实质是先创造子类的实例对象this，然后再将父类的方法添加到this上面（Parent.call(this)）。
ES6的继承机制完全不同，必须先调用super方法
然后再用子类的构造函数修改this。

如果子类没有定义constructor方法，这个对象会被默认添加，代码如下。
也就是说，不管有没有显示定义，任何一个子类都有constructor方法

例外，在子类的构造函数中，只有调用super之后，
才可以使用this关键字，否则会报错。
*/
class Point{
    constructor(x,y){
...
class Bar extends Foo{
    static classMethod(){
        return super.classMethod()+', too';//此时的super指代的是父类
    }
}
...
class Computer extends Human{
    constructor(){
        super();//super此处指代的是父类构造函数
    }
    static pingpong(){
        return super.ping()+' pong';//此处super指代父类
    }
}
